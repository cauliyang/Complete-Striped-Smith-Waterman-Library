...
jstring s_align_cigar_to_jstring(JNIEnv *env, s_align *align) {
  ...
  if (align->cigar != NULL && align->cigarLen > 0) {
    ...
    for (i = 0; i < align->cigarLen; i++) {
      int charsPrinted = sprintf(currentBufferPosition, "%d%c", cigar_int_to_len(align->cigar[i]),
...
jobject s_align_to_ssw_Alignment(JNIEnv *env, s_align *align) {
  ...
  jobject result = (*env)->NewObject(
      env, clazz, constructor, align->score1, align->score2, align->ref_begin1, align->ref_end1,
      align->read_begin1, align->read_end1, align->ref_end2, s_align_cigar_to_jstring(env, align));
  return result;
}
JNIEXPORT jobject JNICALL Java_ssw_Aligner_align(JNIEnv *env, jclass cls, jbyteArray read,
                                                 jbyteArray matrix, jint matrixSize,
                                                 jint score_size, jbyteArray ref, jint gapOpen,
                                                 jint gapExtend, jint flag, jshort filters,
                                                 jint filterd, jint maskLen) {
  ...
  s_align *align
      = ssw_align(profile, refPtr, refLen, gapOpen, gapExtend, flag, filters, filterd, maskLen);
