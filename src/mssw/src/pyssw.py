...
def read(sFile):
    ...
    if ext == "gz" or ext == "gzip":
        with gzip.open(sFile, "r") as f:
            if bFasta == True:
                yield from read_one_fasta(f)
            else:
                yield from read_one_fastq(f)
    else:
        with open(sFile) as f:
            if bFasta == True:
                yield from read_one_fasta(f)
...
def main(args):
    ...
    if args.bSam and args.bHeader and args.bPath:
        ...
        for sRId, sRSeq, _ in read(args.target):
            print(f"@SQ\tSN:{sRId}\tLN:{len(sRSeq)}")
    ...
    for sQId, sQSeq, sQQual in read(args.query):
        ...
        for sRId, sRSeq, _ in read(args.target):
            ...
            if not args.bSam:
                print(
                    f"target_name: {sRId}\nquery_name: {sQId}\noptimal_alignment_score: {resPrint[0]}\t"
                ),
                if resPrint[1] > 0:
                    print(f"suboptimal_alignment_score: {resPrint[1]}\t"),
                if strand == 0:
                    print("strand: +\t"),
                else:
                    print("strand: -\t"),
                if resPrint[2] + 1:
                    print(f"target_begin: {resPrint[2] + 1}\t"),
                print(f"target_end: {resPrint[3] + 1}\t"),
                if resPrint[4] + 1:
                    print(f"query_begin: {resPrint[4] + 1}\t"),
                print(f"query_end: {resPrint[5] + 1}\n")
                if resPrint[-2] > 0:
                    n1 = 1 + resPrint[2]
                    ...
                    for i in range(0, len(sQ), 60):
                        print(f"Target:{n1:>8}\t{sR[i:i+60]}\t{n2}")
                        n1 = n2 + 1
                        n2 = (
                            n2
                            + min(60, len(sR) - i - 60)
                        ...
                        print(f"Query:{n3:>9}\t{sQ[i:i+60]}\t{n4}\n")
                        n3 = n4 + 1
                        n4 = (
                            n4
                            + min(60, len(sQ) - i - 60)
                ...
                if resPrint[0] == 0:
                    ...
                    if strand == 1:
                        ...
                        print("0\t"),
                    print(f"{sRId}\t{resPrint[2]+1}\t{mapq}\t"),
                    print(sCigar),
                    print("\t*\t0\t0\t"),
                    print(
                        sQSeq[resPrint[4] : resPrint[5] + 1]
                    ) if strand == 0 else print(sQRcSeq[resPrint[4] : resPrint[5] + 1]),
                    print("\t"),
                    if sQQual:
                        if strand == 0:
                    ...
                    print(f"\tAS:i:{resPrint[0]}"),
                    print("\tNM:i:{}\t".format(len(sA) - sA.count("|"))),
                    if resPrint[1] > 0:
                        print(f"ZS:i:{resPrint[1]}")
                    else:
                        print
...
if __name__ == "__main__":
    ...
    main(args)
    t2 = ti.default_timer()
    sys.stderr.write(f"CPU time: {t2 - t1} seconds\n")
