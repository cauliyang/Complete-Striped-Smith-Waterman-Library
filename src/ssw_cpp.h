...
struct Alignment {
  ...
  int32_t
      ref_end_next_best;    // Reference end position of the next best alignment
...
struct Filter {
  // NOTE: No matter the filter, those five fields of Alignment will be given
  // anyway.
  ...
  bool report_begin_position; // Give ref_begin and query_begin.
                              //   If it is not set, ref_begin and query_begin
                              //   are -1.
  ...
  Filter()
      : report_begin_position(true), report_cigar(true), score_filter(0),
        distance_filter(32767){};

  Filter(const bool &pos, const bool &cigar, const uint16_t &score,
         const uint16_t &dis)
      : report_begin_position(pos), report_cigar(cigar), score_filter(score),
        distance_filter(dis){};
};
...
public:
  ...
  // alignment ending position will >= maskLen. We suggest to
  //                     use readLen/2, if you don't have special concerns.
  //                     Note: maskLen has to be >= 15, otherwise this function
...
private:
  ...
  int TranslateBase(const char *bases, const int &length,
