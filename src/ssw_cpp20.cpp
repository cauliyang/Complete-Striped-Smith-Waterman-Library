...
void BuildSwScoreMatrix(uint8_t match_score, uint8_t mismatch_penalty,
                        int8_t *matrix) {
...
void ConvertAlignment(const s_align &s_al, const int &query_len,
  ...
  if (s_al.cigarLen > 0) {
    ...
    for (int i = 0; i < s_al.cigarLen; ++i) {
      ...
      cigar_string << cigar_int_to_len(s_al.cigar[i])
                   << cigar_int_to_op(s_al.cigar[i]);
    }
...
void CleanPreviousMOperator(bool *in_M, bool *in_X, uint32_t *length_M,
                            uint32_t *length_X,
...
int CalculateNumberMismatch(StripedSmithWaterman::Alignment *al,
                            int8_t const *ref, int8_t const *query,
                            int query_len) {
  ref += al->ref_begin;
  ...
  for (unsigned int i = 0; i < al->cigar.size(); ++i) {
    ...
    if (op == 'M') {
      ...
      CleanPreviousMOperator(&in_M, &in_X, &length_M, &length_X, &new_cigar,
                             &new_cigar_string);
...
Aligner::Aligner(uint8_t match_score, uint8_t mismatch_penalty,
                 uint8_t gap_opening_penalty, uint8_t gap_extending_penalty)
    : match_score_(match_score), mismatch_penalty_(mismatch_penalty),
...
Aligner::Aligner(const int8_t *score_matrix, int score_matrix_size,
  ...
  std::copy(score_matrix,
            score_matrix + score_matrix_size_ * score_matrix_size_,
...
bool Aligner::Align(const char *query, const Filter &filter,
  ...
  s_profile *profile =
      ssw_init(translated_query.data(), query_len, score_matrix_.data(),
               score_matrix_size_, score_size);

  uint8_t flag = 0;
  SetFlag(filter, &flag);
  s_align *s_al =
      ssw_align(profile, translated_reference_.data(), reference_length_,
  ...
  alignment->mismatches =
      CalculateNumberMismatch(alignment, translated_reference_.data(),
...
bool Aligner::Align(const char *query, const char *ref, int ref_len,
  ...
  SetFlag(filter, &flag);
  s_align *s_al =
      ssw_align(profile, translated_ref.data(), valid_ref_len,
                gap_opening_penalty_, gap_extending_penalty_, flag,
                filter.score_filter, filter.distance_filter, maskLen);
  ...
  alignment->mismatches = CalculateNumberMismatch(
      alignment, translated_ref.data(), translated_query.data(), query_len);
...
bool Aligner::ReBuild(uint8_t match_score, uint8_t mismatch_penalty,
